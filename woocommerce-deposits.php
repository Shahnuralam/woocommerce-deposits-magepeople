...
if (wc_deposits_woocommerce_is_active()) :
    ...
    class WC_Deposits
    {
        ...
        {
            define('WC_DEPOSITS_VERSION', '1.3.3');
            ...
            if (version_compare(PHP_VERSION, '7.0.0', '<')) {
                ...
                if (is_admin()) {
                    ...
                    $this->enqueue_admin_notice(sprintf(esc_html__('%s Requires PHP version %s or higher.'), esc_html__('WooCommerce Deposits', 'Advanced Partial Payment and Deposit For Woocommerce'), '5.6'), 'error');
                }
            ...
            if (is_admin()) {
                ...
                add_action('init', 'MagePeople\WCDP\WC_Deposits::plugin_activated', 100); //plugin activated is not called with automatic updates anymore.
        ...
        function process_update()
        {
            ...
            if (version_compare(get_option('wc_deposits_db_version', '3.0.0'), '4.0.0', '<')) {
                ...
                if ($deposit_products->post_count == 0) {
                    ...
                    $this->enqueue_admin_notice(esc_html__('WooCommerce Deposits database update complete', 'Advanced Partial Payment and Deposit For Woocommerce'), 'info', true);

                    return;
                }
                $this->enqueue_admin_notice(esc_html__('WooCommerce Deposits database update running in the background', 'Advanced Partial Payment and Deposit For Woocommerce'), 'info', true);
        ...
        function check_version_disable()
        {
            if (function_exists('WC') && version_compare(WC()->version, '3.7.0', '<')) {
                ...
                if (is_admin()) {
                    add_action('admin_notices', array($this, 'show_admin_notices'));
                    $this->enqueue_admin_notice(sprintf(esc_html__('%s Requires WooCommerce version %s or higher.'), esc_html__('WooCommerce Deposits', 'Advanced Partial Payment and Deposit For Woocommerce'), '3.7.0'), 'error');
                }
            }
        ...
        function register_wcdp_payment_post_type()
        {
            ...
            wc_register_order_type(
                ...
                array(
                    ...
                    'labels' => array(
                        'name' => esc_html__('Partial Payments', 'Advanced Partial Payment and Deposit For Woocommerce'),
                        'singular_name' => esc_html__('Partial Payment', 'Advanced Partial Payment and Deposit For Woocommerce'),
                        'edit_item' => esc_html_x('Edit Partial Payment', 'custom post type setting', 'Advanced Partial Payment and Deposit For Woocommerce'),
                        'search_items' => esc_html__('Search Partial Payments', 'Advanced Partial Payment and Deposit For Woocommerce'),
                        'parent' => esc_html_x('Order', 'custom post type setting', 'Advanced Partial Payment and Deposit For Woocommerce'),
                        'menu_name' => esc_html__('Partial Payments', 'Advanced Partial Payment and Deposit For Woocommerce'),
                    ),
                    'public' => false,
                    'show_ui' => true,
...
//                    'exclude_from_order_sales_reports' => true,
        ...
        function plugin_row_meta($links, $file)
        {
            ...
            if ($file === 'Advanced Partial Payment and Deposit For Woocommerce/Advanced Partial Payment and Deposit For Woocommerce.php') {

                $row_meta = array(
                    'settings' => '<a href="' . esc_url(admin_url('/admin.php?page=wc-settings&tab=wc-deposits&section=auto_updates')) . '"> ' . esc_html__('Settings', 'Advanced Partial Payment and Deposit For Woocommerce') . '</a>',
                    'documentation' => '<a  target="_blank" href="' . esc_url('#') . '"> ' . esc_html__('Documentation', 'Advanced Partial Payment and Deposit For Woocommerce') . '</a>',
                    'support' => '<a target="_blank" href="' . esc_url('#') . '"> ' . esc_html__('Support', 'Advanced Partial Payment and Deposit For Woocommerce') . '</a>',
                );

                $row_meta['view-details'] = sprintf('<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
                    esc_url(network_admin_url('plugin-install.php?tab=plugin-information&plugin=' . 'Advanced Partial Payment and Deposit For Woocommerce' . '&TB_iframe=true&width=600&height=550')),
                    sprintf(esc_html__('More information about %s', 'Advanced Partial Payment and Deposit For Woocommerce'), 'WooCommerce Deposits'),
                    esc_attr('WooCommerce Deposits'),
                    esc_html__('View details', 'Advanced Partial Payment and Deposit For Woocommerce')
                );
        ...
        public function load_plugin_textdomain()
        {
            ...
            load_plugin_textdomain('Advanced Partial Payment and Deposit For Woocommerce', false, dirname(plugin_basename(__FILE__)) . '/locale/');
        }
        ...
        public function enqueue_styles()
{
    ...
    if (is_array($this->admin_notices) && !empty($this->admin_notices)) {
        ...
        public function register_order_status()
        {
            ...
            register_post_status('wc-partially-paid', array(
                'label' => _x('Partially Paid', 'Order status', 'Advanced Partial Payment and Deposit For Woocommerce'),
                'public' => true,
                'exclude_from_search' => false,
                'show_in_admin_all_list' => true,
                'show_in_admin_status_list' => true,
                'label_count' => _n_noop('Partially Paid <span class="count">(%s)</span>',
                    'Partially Paid <span class="count">(%s)</span>', 'Advanced Partial Payment and Deposit For Woocommerce')
            ));
    ...
    register_activation_hook(__FILE__, array('\MagePeople\WCDP\WC_Deposits', 'plugin_activated'));
    register_deactivation_hook(__FILE__, array('\MagePeople\WCDP\WC_Deposits', 'plugin_deactivated'));
