...
class WC_Deposits_Emails
{
    ...
    public function __construct()
    {
        ...
        if(function_exists('wcdp_partial_payment_complete_order_status')){
            $partial_payment_status = wcdp_partial_payment_complete_order_status();
        }

        $full_payment_status = get_option('wc_deposits_order_fully_paid_status','processing');
        ...
        add_filter('woocommerce_email_enabled_new_order', array($this, 'disable_wcdp_payment_emails'), 999, 2);
        add_filter('woocommerce_email_enabled_customer_on_hold_order', array($this, 'disable_wcdp_payment_emails'), 999, 2);
        add_filter('woocommerce_email_enabled_customer_processing_order', array($this, 'disable_wcdp_payment_emails'), 999, 2);
        add_filter('woocommerce_email_enabled_customer_completed_order', array($this, 'disable_wcdp_payment_emails'), 999, 2);
    }
    ...
    function disable_wcdp_payment_emails($enabled, $order)
    {
    ...
    function original_order_summary($order, $sent_to_admin = false, $plain_text = false, $email = '')
    {
        ...
        ?> <p> <?php echo esc_html__('Below is a summary of original order', 'Advanced Partial Payment and Deposit For Woocommerce') ?>
    ...
    public function partial_payment_details($order, $sent_to_admin = false, $plain_text = false, $email = '')
    {
        ...
        if(!apply_filters('wc_deposits_email_show_partial_payments_summary',true,$order,$email,$sent_to_admin)) return;
