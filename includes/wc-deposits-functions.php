...
use MagePeople\WCDP\WC_Deposits_Admin_Order;
...
function wc_deposits_deposit_breakdown_tooltip()
{
    ...
    if ($display_tooltip && isset(WC()->cart->deposit_info['deposit_breakdown']) && is_array(WC()->cart->deposit_info['deposit_breakdown'])) {
        ...
        $labels = apply_filters('wc_deposits_deposit_breakdown_tooltip_labels', array(
            'cart_items' => esc_html__('Cart items', 'Advanced Partial Payment and Deposit For Woocommerce'),
            'fees' => esc_html__('Fees', 'Advanced Partial Payment and Deposit For Woocommerce'),
            'taxes' => esc_html__('Tax', 'Advanced Partial Payment and Deposit For Woocommerce'),
            'shipping' => esc_html__('Shipping', 'Advanced Partial Payment and Deposit For Woocommerce'),
        ));
...
function wcdp_status_widget_partially_paid()
{
    ...
    <li class="partially-paid-orders">
        <a href="<?php echo admin_url('edit.php?post_status=wc-partially-paid&post_type=shop_order'); ?>">
            <?php
            printf(
                _n('<strong>%s order</strong> partially paid', '<strong>%s orders</strong> partially paid', $partially_paid_count, 'Advanced Partial Payment and Deposit For Woocommerce'),
...
function WCDP()
{
    return \MagePeople\WCDP\WC_Deposits::get_singleton();
}
