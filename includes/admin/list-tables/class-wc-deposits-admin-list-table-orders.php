...
class WC_Deposits_Admin_List_Table_Orders {
    ...
    function add_has_deposit_column($columns){
        ...
        foreach($columns as $key => $column){
            ...
            if($key === 'order_total'){
                $new_columns['wc_deposits_has_deposit'] = esc_html__('Has Deposit','Advanced Partial Payment and Deposit For Woocommerce');
            }
        ...
        if ( 'wc_deposits_has_deposit' === $column ) {
            ...
            if($order){
                ...
                if($order_has_deposit === 'yes'){
                    echo '<span class="button wcdp_has_deposit">&#10004; '.esc_html__('Yes','Advanced Partial Payment and Deposit For Woocommerce').'</span>';
                } else {
                    echo '<span class="button wcdp_no_deposit"> &#10006; '.esc_html__('No','woocommerce').'</span>';
                }
            }
        }
	...
	public function order_bulk_actions( $actions ) {
		$actions['mark_partially_paid'] = esc_html__( 'Mark partially paid', 'Advanced Partial Payment and Deposit For Woocommerce' );
		return $actions;
	}
	...
	function handle_bulk_actions( $redirect_to, $action, $ids ) {
 		if( $action == 'mark_partially_paid' ) {
			...
			foreach ( $ids as $id ) {
				...
				$order->update_status( 'partially-paid', esc_html__( 'Order status changed by bulk edit:', 'Advanced Partial Payment and Deposit For Woocommerce' ) );
				$changed++;
			}
    ...
    function bulk_admin_notices() {
		...
		if ( isset( $_REQUEST['marked_partially_paid'] ) ) {
			...
			if ( 'edit.php' == $pagenow && 'shop_order' == $post_type ) {
				$message = sprintf( _n( 'Order status changed.', '%s order statuses changed.', $number,'Advanced Partial Payment and Deposit For Woocommerce' ), number_format_i18n( $number ) );
