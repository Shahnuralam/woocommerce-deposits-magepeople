 ...
 *
 */
class WCDP_Payment extends WC_Order
{
    ...
    public function get_order_item_totals($tax_display = '')
    {
        ...
        if($this->get_meta('_wc_deposits_partial_payment_itemized') === 'yes'){
            return parent::get_order_item_totals($tax_display);
        }

        if (did_action('woocommerce_order_details_after_order_table_items')) {

            return array(array(
                'label' => esc_html__('Partial Payment amount:', 'Advanced Partial Payment and Deposit For Woocommerce'),
    ...
    {
        //skip the calculations on saving the partial payment
        if(strpos(wp_debug_backtrace_summary(), 'WC_Admin_Meta_Boxes->save_meta_boxes') === false){
        return parent::calculate_totals($and_taxes);
        }
    }
